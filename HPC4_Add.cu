...
int main()
{
    ...
    // This means that the CPU and GPU share memory locations rather than having distinct RAM allocations and types.
    ...
    // copy: Blocks the CPU until the copy is complete
    // Copy begins when all preceding CUDA calls have completed
    // cudaMemcpyHostToDevice: device/destination pointer to host/source pointer
    ...
    // dim3: dim3 is an integer vector type based on uint3 that is used to specify dimensions.
